...
import static com.diffusiondata.gateway.example.activity.feed.ActivityFeedSnapshotPollingSourceHandlerImpl.DEFAULT_POLLING_TOPIC_PATH;
...
class ActivityFeedSnapshotPollingSourceHandlerImplTest {
    @Mock
    ...
    void beforeEachTest() {
        ...
        handler = new ActivityFeedSnapshotPollingSourceHandlerImpl(
        ...
        final String topicPrefix =
            ((ActivityFeedSnapshotPollingSourceHandlerImpl) handler)
                .getTopicPath();
    ...
    void testPollWhenServiceStateIsActiveAndLatestActivitiesIsEmpty() {
        final Collection<Activity> activities = List.of();

        when(stateHandlerMock.getState())
            .thenReturn(ServiceState.ACTIVE);

        when(activityFeedClientMock.getLatestActivities())
